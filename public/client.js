...
socket.on("signal", async msg => {
  ...
  if (isHost()) {
    ...
    if (msg.sdp) {
      ...
      catch(e){ if(!candidateQueues[cameraId]) candidateQueues[cameraId]=[]; candidateQueues[cameraId].push(msg.candidate); }
    }
  }
});
...
function createHostPc(cameraId){
  ...
  pc.ontrack = e => {
    ...
    if(!currentCameraId) { switchToCamera(cameraId); cameraIndex=cameraOrder.indexOf(cameraId); }
    if(candidateQueues[cameraId]) { candidateQueues[cameraId].forEach(async c=>{ try{ await pc.addIceCandidate(new RTCIceCandidate(c)); }catch{} }); candidateQueues[cameraId]=[]; }
  };
  hostPcs[cameraId]=pc;
}
...
function renderUserList(users){
  userList.innerHTML=""; cameraOrder=[];
  users.forEach(u=>{
    if(u.role==="camera"){
      const btn=document.createElement("div");
      btn.className="camera-btn"; btn.dataset.id=u.id;
      ...
      userList.appendChild(btn); cameraOrder.push(u.id);
    }
  });
  updateActiveButtons();
}
...
function switchToCamera(cameraId){
  if(!hostStreams[cameraId]){ currentCameraId=cameraId; camLabel.textContent="Connecting..."; updateActiveButtons(); return; }
