...
function switchToCamera(cameraId){
  ...
  const stream = hostStreams[cameraId];
  if(stream){
    remoteVideo.srcObject = null;
    setTimeout(()=>{ remoteVideo.srcObject = stream; remoteVideo.play().catch(()=>{}); },0);
  } else camLabel.textContent="Connecting to "+cameraId.slice(0,6);
